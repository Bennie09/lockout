    ...
    <Screen scroll extraBottom={40}>
      ...
      <Card hot={status.locked}>
        ...
        <Type variant="title" style={{ marginTop: 6 }}>
          {status.locked ? 'This one stays closed.' : 'You can start a session.'}
        </Type>
        ...
        <Button
          label={status.locked ? 'I still want in' : 'Start a session'}
          style={{ marginTop: 14 }}
          variant={status.locked ? 'danger' : 'primary'}
          onPress={() => router.push(status.locked ? `/locked/${app.id}` : `/session/${app.id}`)}
        />
      </Card>

      <SectionHeader title="Daily cap" aside={formatDuration(app.usedMinutesToday) + ' used'} />
      ...
      <Type variant="caption">When the cap is hit, Lockout treats it like a window until midnight.</Type>
